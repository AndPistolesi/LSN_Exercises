...
void Input(void)
{
  ...
  box = pow(vol,1.0/3.0);
  bin_size = (double)(box/2)/ivs;
  cout << "Volume of the simulation box = " << vol << endl;
  cout << "Edge of the simulation box = " << box << endl;

  for(int i=0; i<ivs; i++) 
    Dv[i] = 4*M_PI/3 * ( pow( bin_size*(i+1),3 ) - pow( bin_size*i,3) );
  ...
  Wtail = 32*M_PI*rho*(1/(9*pow(rcut,9))-1/(6*pow(rcut, 3)));
  ...
  for(int i=0; i<ivs; i++) ig[i] = i + n_props;
  ...
  cout << "Initial pressure         = " << rho*walker[it] + walker[iw] << endl;
...
// distance i-j in pbc
  ...
  walker[ie] = 4.0 * v + Vtail + kin;  // Total energy;
  walker[iw] = 1/(3*vol)*48*(w + Wtail); //virial
  Epot0[count] = 4.0 * v + Vtail;
  for(int i=0; i<ivs; i++) 
    walker[ig[i]] = (double)g_r[i]/(rho * npart * Dv[i]);
...
void Averages(int iblk) //Print results for current block
{
   ...
   ofstream Epot, Ekin, Etot, Temp, Pres, Epot1, G_r, G_r_fin;
   const int wd=12;
    ...
    cout << "Acceptance rate " << accepted/attempted << endl << endl;
      ...
      G_r.open("G_r.dat", ios::app);
      G_r_fin.open("G_r_fin.dat", ios::app);
      Epot1.open("epot.dat", ios::app);
    ...
    stima_pres = rho*stima_temp + blk_av[iw]/blk_norm; //Pressure
    ...
    for(int i=0; i<ivs;++i) {
      ...
      stima_g[i] = (double)blk_av[ig[i]]/blk_norm;
      glob_av[ig[i]] += stima_g[i];
      glob_av2[ig[i]] += stima_g[i]*stima_g[i];
...
//Instantaneus potential energy1.
  for (int i=0; i<Epot0.size(); i++)
    Epot1 << setw(wd) << i << setw(wd) << Epot0[i] << endl;
...
//final g(r)
  if(iblk == nblk){
    ...
    for(int i=0; i<ivs; i++)  
      G_r_fin << i*bin_size << setw(wd) << (double)glob_av[ig[i]]/iblk << setw(wd) << err_g_r[i] << endl; 
  }
