...
Misura Cum_av(vector<double> x, double(*g)(double) /*=const_1*/, double(*f)(double) /*=Id*/){
...
void Print_block(vector<Misura> b, ofstream& out){
...
void Print_block(Misura b, ofstream& out){
...
vector<Misura> Block_v(int nstep, int nblk, Pos* P, bool print /*1*/, string save /*""*/){
	ofstream out;
	if(save != ""){out.open(save);} 
	vector<double> w;
	w =  P->Measure_v();
	vector<vector<double>> v(w.size(), vector<double> (nblk));
	vector<Misura> m(w.size());	
...
Misura Block(int nstep, int nblk, Pos* P, bool print /*1*/, string save /*""*/){
	ofstream out;
	if(save != ""){out.open(save, ios::app);} 
	double w;
